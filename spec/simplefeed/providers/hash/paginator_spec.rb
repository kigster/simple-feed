...
RSpec.describe SimpleFeed::Providers::Hash::Paginator do
  ...
  context '#events' do
    ...
    context '#events' do
      ...
      let(:generated_response) { SimpleFeed::Consumer::Response.new({ consumer_id => events.map(&:dup).dup }) }
      ...
      it 'should correctly paginate events' do
        manually_sorted_events.each_with_index do |event, index|
          page = index + 1
          response = paginated.paginate(consumers: [consumer_id], page: page, per_page: 1)
          expect(response[consumer_id]).to eq([event])
        end
        expect(paginated.paginate(consumers: [consumer_id], page: 1, per_page: 2)[consumer_id]).to eq [events[1], events[2]]
      end
    end
  end
end
