  ...
  module Providers
    module Redis
      ...
      class Provider < ::SimpleFeed::Providers::Base::Provider
        ...
        #   users = Users.new(provider: provider, consumers: opts.delete(:consumers))
        #   opts.empty? ?
        ...
        def publish(consumers:, data:, at: Time.now)
          with_response_pipelined(consumers) do |redis, key|
        ...
        def delete_if(consumers:)
          raise ArgumentError, '#delete_if must be called with a block that receives (consumer_id, event) as arguments.' unless block_given?

          with_response_batched(consumers) do |key|
            fetch(consumers: [key.consumer_id])[key.consumer_id].map do |event|
              with_redis do |redis|
        ...
        def paginate(consumers:, page:,
                     per_page: feed.per_page,
                     with_total: false,
                     reset_last_read: false)

          reset_last_read_value(consumers: consumers, at: reset_last_read) if reset_last_read
        ...
        def fetch(consumers:, since: nil, reset_last_read: false)
          if since == :unread
            last_read_response = with_response_pipelined(consumers) do |redis, key|
              get_users_last_read(redis, key)
            end
          end

          response = with_response_pipelined(consumers) do |redis, key|
            if since == :unread
              redis.zrevrangebyscore(key.data, '+inf', (last_read_response.delete(key.consumer_id) || 0).to_f, withscores: true)
            elsif since
        ...
        def unread_count(consumers:)
          ...
          with_response_pipelined(response.consumers, response) do |redis, key, _response|
            last_read = _response.delete(key.consumer_id).to_f
        ...
        def transform_response(consumer_id = nil, result)
          case result
            when ::Redis::Future
              transform_response(consumer_id, result.value)
            ...
            when ::Array
              ...
              if result.any? { |v| transformable_type?(v) }
                result = result.map { |v| transform_response(consumer_id, v) }
              end

              if result.size == 2 && result[1].is_a?(Float)
                SimpleFeed::EventTuple.new(data: result[0], at: Time.at(result[1]))
              else
                result
              end
        ...
        def with_response_pipelined(consumers, response = nil)
          with_response(response) do |response|
            batch_pipelined(consumers) do |redis, key|
              response.for(key.consumer_id) { yield(redis, key, response) }
            end
          end
        end

        def with_response_multi(consumers, response = nil)
          with_response(response) do |response|
            batch_multi(consumers) do |redis, key|
              response.for(key.consumer_id) { yield(redis, key, response) }
            end
          end
        end
