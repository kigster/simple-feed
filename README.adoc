 ...
 * Store consumer_id as either a string or an integer (the latter is base62 encoded when serialized, so very large users_ids are OK)
...
Now you use partioning scheme: make "consumer_id" a combination `iso_country_code.postal_code`, eg for San Francisco, you'd use `us.94107`, but for Australia you could use, eg `au.3148`.
...
# or equivalent:
@event = SimpleFeed::EventTuple.new('hello', Time.now)
...
# => [ <SimpleFeed::EventTuple#0x2134afa value='hello' at='2016-11-20 23:32:56 -0800'> ]
----
...
activity = SimpleFeed.followers.activity(consumer_id)         # => [... complex object removed for brevity ]
...
activity.paginate(page: 1, reset_last_read: true)
# [
#     [0] #<SimpleFeed::EventTuple: value=good bye, at=1480475294.0579991>,
#     [1] #<SimpleFeed::EventTuple: value=hello, at=1480475294.057138>
# ]
...
activity.delete_if do |event, consumer_id|
  event.value =~ /good/
end
# => [
#     [0] #<SimpleFeed::EventTuple: value=good bye, at=1480475294.0579991>
...
be used as a hash (keyed on consumer_id) to fetch the result of a given
user.
...
@event_producer.followers.find_in_batches do |group|
  ...
  # => [Response] { consumer_id1 => [Boolean], consumer_id2 => [Boolean]... }
...
* `#event(value, at)` returns a fully constructed `SimpleFeed::EventTuple` instance
...
For completeness sake we'll show the multi-user API responses only. For a single-user use-case the response is typically a scalar, and the input is a singular `consumer_id`, not an array of ids.
...
responses for each user, accessible via `response[consumer_id]` method.
...
# => [Response] { consumer_id => [deleted_event1, deleted_event2, ...], ... }
...
# => [Response] { consumer_id => [Array]<EventTuple>, ... }
# Options:
...
#        => [Response] { consumer_id => { events: Array<EventTuple>, total_count: 3 }, ... }
