...
Pushing events to the feed requires three things:

 * String `data`
 * Float `at` (typically, the timestamp)
 * One or more user objects or user_id, or consumers: basically — who should see this feed.
...
=== Consuming Events (Reading)

You can consume events for one user or multiple users simultaneously.

 * Use methods such as `paginate`, `fetch`, `delete_if`, `wipe`, etc.

 * Reading feed for one user (or one type of user) is a O(1) operation

 * For each activity (user) read the counts `total_count` and `unread_count` — the number of total and new items in the feed, where `unread_count` is computed since the user last reset their `read status`.

 * The `last_read` timestamp can be automatically reset when the user is shown the feed via `paginate` method (controlled via a parameter).

 * You can delete items from user's feed selectively (for instance, if a user un-follows someone they shouldn't see their events anymore).
